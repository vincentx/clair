...
VncClient.prototype.framebufferUpdate = function(numberOfRectangles) {
	...
	return function(message) {
		...
		if (encoding == 0) {
			var size = width * height * protocol._server.pixelFormat.bitsPerPixel / 8;
			protocol._handler = protocol.collect(size, new Buffer(size), protocol.rawEnconding(x, y, width, height));
		}
	}
}

VncClient.prototype.rawEnconding = function(x, y, width, height) {
	var protocol = this;
	return function(pixels) {
		protocol.emit('rawEncoding', x, y, width, height, pixels);
		protocol._handler = protocol.serverToClient;
	}
}
...
VncClient.prototype.collect = function(size, buffer, callback) {
	var protocol = this;
	return function(message) {
		if (buffer._index + message.size() < size) {
			buffer.append(message, message.size());
			protocol._handler = protocol.collect(size - message.size(), buffer, callback);
		} else {
			buffer.append(message, message.size());
			callback(buffer);
		}
