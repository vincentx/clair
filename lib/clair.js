...
var sys = require('sys'), net = require('net'), vnc = require('vnc.js');
var Png = require('png').Png;var fs  = require('fs');var Jpeg = require('jpeg').Jpeg;
...
function toAscii(data) { var ascii = new Buffer(data.length); for (var i = 0; i < data.length; i++) ascii[i] = data[i];return ascii;}
function toBuffer(data) { var ascii = new Buffer(data.length); for (var i = 0; i < data.length; i++) ascii[i] = data.charCodeAt(i);return ascii;}


var port = 5901, host = 'ec2-79-125-31-203.eu-west-1.compute.amazonaws.com', socket = null;
// var port = 5900, host = 'localhost', socket = null;

var client = vnc.createClient();
client.on('protocolVersion', function(message) { sys.log('protocol version...'); socket.write("RFB 003.003\n");});
client.on('security', function(message) { 
	sys.log('security...');
	var mode = message.read32(); 
	var challenge = [];
	for (var i = 0; i < 16; i ++) {challenge.push(message.read8());}
	socket.write(toAscii(DES.encrypt('3mapw9$n', challenge)));
});

client.on('securityResult', function(message) {
	sys.log('security result : ' + message.read32());
	socket.write('\1');
});

client.on('serverInit', function(message) {
	sys.log('server init ... ');
	socket.write(toBuffer('\2\0\0\1\0\0\0\0'));	
	socket.write(toBuffer('\3\0\0\0\0\0\0\9\0\9'));
});

client.on('serverToClient', function(message) {
	sys.log('server send message to client ... ');	
});

client.on('rawEncoding', function(x, y, width, height, pixels) {
	sys.log('raw encoding');
	var jpg = new Jpeg(pixels, width, height, "bgra");
	var jpg_image = jpg.encodeSync();
	// var png = new Png(pixels, width, height, "bgr");
	// var png_image = png.encodeSync();
	
	fs.writeFileSync('./'+ x + '.' + y + '.jpg', jpg_image.toString('binary'), 'binary');
	// fs.writeFileSync('./'+ x + '.' + y + '.png', png_image.toString('binary'), 'binary');	
});
