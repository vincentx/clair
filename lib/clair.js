...
client.on('security', function(message) { 
	...
	socket.write(toAscii(DES.encrypt('p@55w0rd', challenge)));
});
...
client.on('serverInit', function(message) {
	...
	socket.write(toBuffer('\3\0\0\0\0\0\5\240\3\204'));
});
...
client.on('rawEncoding', function(x, y, width, height, pixels) {
	var png = new Jpeg(pixels, width, height);
	var png_image = png.encodeSync();
	fs.writeFileSync('./'+ x + '.' + y + '.jpg', png_image.toString('binary'), 'binary');
	fs.writeFileSync('./'+ x + '.' + y + '.raw', pixels.toString('binary'), 'binary');
});
